...
function asyncify(fn: Function): any {
  let isAsync = false;
  return function () {
...
async function handler(req: any, res: any) {
  try {
    ...
    let func = ((req: any, res: any) => {throw new Error('Default function is being invoked, something went wrong.')})
    ...
    console.log('## RESPONSE: ' + JSON.stringify(lambdaRes,null,2))
    return lambdaRes
  } catch(error) {
