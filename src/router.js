...
function defaultHandler(req,res) {
  ...
  if (req.headers['accept'].includes('text/html')) {
    ...
    let readmeFilename = path.resolve(absDir,'README.md')
    try {
...
function nameToRegex(fileName){
  ...
  if (pattern.indexOf(':') != -1) {
    pattern = pattern.split('/').map((e) => {
      ...
      let varGrabber = /:(?<varName>[A-Za-z0-9_-]+)/mg
      let match = varGrabber.exec(e)
...
function resolveHandler(req, srcRoot='../test-app') {
  ...
  routes.some((r) => {
    let match = r.regex.exec(req.path)
    if (match) {
